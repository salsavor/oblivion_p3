...
endpoints.getMyFavoritos = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      status: "success",
      message: "Favoritos do utilizador.",
      data: dados,
    });
  } catch (error) {
    console.error("[favorito] Erro:", error);
...
endpoints.addFavorito = async (req, res) => {
  ...
  if (!tipo_alvo || !alvo_id) {
    return res.status(400).json({
      status: "error",
      message: "tipo_alvo e alvo_id são obrigatórios.",
      data: null,
    });
  }
  ...
  try {
    ...
    const existe = await Favorito.findOne({
      where: { userId, tipo_alvo, alvo_id },
    });
    if (existe) {
...
// deletar os favoritos do user autenticado
endpoints.removeFavorito = async (req, res) => {
  ...
  try {
    ...
    if (favorito.userId !== userId && !req.decoded.admin) {
      return res
        .status(403)
        .json({ status: "error", message: "Sem permissão.", data: null });
    }
...
endpoints.getFavoritosByUser = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      status: "success",
      message: `Favoritos do utilizador #${userId}.`,
      data: dados,
    });
  } catch (error) {
    console.error("[favorito] Erro:", error);
    return res.status(500).json({
      status: "error",
      message: "Erro ao listar favoritos do utilizador.",
      data: null,
    });
  }
};
